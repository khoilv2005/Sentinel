...
async function renderHost(id) {
  ...
  content.innerHTML = `
    <div class="page-stack">
      ...
      <div class="stat-grid">
        ${statCard('CPU', metricValue(t?.cpu_usage_percent,'%'), 'Latest telemetry', t?.cpu_usage_percent >= 95 ? 'red' : t?.cpu_usage_percent >= 80 ? 'yellow' : 'green')}
...
function credentialProfileModal(onSaved) {
  openModal('Create credential profile','Secrets are encrypted before they are stored.',`<div id="agentless-credential-form" class="form-stack"><div class="form-grid"><label>Name<input id="agentless-cred-name" required placeholder="Windows monitoring"/></label><label>Type<select id="agentless-cred-type"><option value="winrm">Windows / WinRM</option><option value="ssh_password">Linux / SSH password</option><option value="ssh_key">Linux / SSH private key</option><option value="snmp_v2">SNMP v2c</option><option value="snmp_v3">SNMP v3</option></select></label><label>Username<input id="agentless-cred-user" placeholder="monitoring-user"/></label><label>Secret / password / community<textarea id="agentless-cred-secret" required></textarea></label><label>Secondary secret<textarea id="agentless-cred-secret2" placeholder="SNMPv3 privacy password (optional)"></textarea></label><label>Options JSON<textarea id="agentless-cred-options">{}</textarea></label></div><button id="save-agentless-credential" class="button primary" type="button">Save encrypted credential</button></div>`);
  $('#save-agentless-credential').onclick=async()=>{const form=$('#agentless-credential-form');const invalid=form.querySelector(':invalid');if(invalid){invalid.focus();return;}try{let options={};try{options=JSON.parse($('#agentless-cred-options').value||'{}');}catch{throw new Error('Options must be valid JSON');}const created=await api('/api/v1/credentials',{method:'POST',body:JSON.stringify({name:$('#agentless-cred-name').value.trim(),credential_type:$('#agentless-cred-type').value,username:$('#agentless-cred-user').value.trim()||null,secret:$('#agentless-cred-secret').value,secondary_secret:$('#agentless-cred-secret2').value||null,options})});closeModal();toast('Credential profile saved');await onSaved(created);}catch(e){toast(e.message,true);}};
}
...
async function renderAgentless() {
  ...
  $('#new-agentless-credential').onclick=()=>credentialProfileModal(async created=>{await renderAgentless();$('#al-credential').value=created.id;});
