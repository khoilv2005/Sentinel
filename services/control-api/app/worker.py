...
def run_job(job_id: str):
    """Run one network discovery job.

    Discovery is deliberately inventory-only. It may create/update asset identity
    and observation metadata, but it must not decide operational health. Health is
    owned by configured monitoring methods (managed agent, WinRM, SSH, SNMP or
    reachability collectors). This avoids scan timing overriding live collector
    state.
    """
    db = SessionLocal()
    try:
        ...
        # Absence from a discovery scan does not mark an asset down.
        job.status = "complete"
        job.discovered = found
